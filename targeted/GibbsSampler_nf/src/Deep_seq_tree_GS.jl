...
##
## This copy carries three changes relative to the project version, all confined
## to deep_seq_GS. The helper functions are unchanged.
##
##   1. Block 2 crash guard. The slack passed to Uniform is now checked: when it
##      is zero or has gone slightly negative through floating-point rounding the
##      windows are pinned to the current mutation extremes instead of calling
##      Uniform(0, x) with x <= 0, which throws. On a normal run, where the slack
##      is always strictly positive, this path is never taken and no random
##      numbers are drawn on the guarded branch, so the output is unchanged.
##
##   2. Performance: the branch and node traversal orders are materialised once
##      before the iteration loop rather than rebuilt every iteration. The order
##      is identical to the original per-iteration calls and no random numbers are
##      consumed, so the output is unchanged.
##
##   3. Performance: the proposal distribution built for sampling is reused for
##      its log-density evaluation rather than being constructed a second time
##      with identical parameters. The random draw and every log-density value
##      are unchanged, so the output is unchanged.
#####
using Phylo
using RCall
using DataFrames
...
function deep_seq_GS(tree, start_VAFs, end_VAFs, muts, iter, burn_in, thin;
    ...
    # Pre-compute the traversal orders once. These are deterministic for a fixed
    # tree and consume no random numbers, so reusing them across iterations gives
    # identical results to rebuilding them each iteration, at lower cost.
    postorder_nodes = collect(traversal(tree, postorder))
    preorder_nodes = collect(traversal(tree, preorder))
    branches = collect(branchiter(tree))

    # Set initial conditions - split values for internal nodes
    for i in postorder_nodes
        if isroot(tree,i)
    ...
    for i in 1:iter
        ...
        for j in branches
            for k in muts[j]
                ...
                # The proposal distribution is built once and reused below for its
                # log-density; this is the same object the original code rebuilt,
                # so the random draw and log-density are identical.
                old_VAF = curr_VAFs[j][k.ID]
                v2 = old_VAF * scale_pm / (1-old_VAF)
                prop_dist = truncated(Beta(v2, scale_pm), br_ends[j], br_starts[j])
                new_VAF = rand(prop_dist)
                if new_VAF<=0
                    new_VAF=min_VAF
                end
                ...
                rev_dist = truncated(Beta(v1, scale_pm), br_ends[j], br_starts[j])
                log_AR = logpdf(Binomial(k.obs_depth, new_p), k.obs_var) -
                        logpdf(Binomial(k.obs_depth, old_p), k.obs_var) +
                        logpdf(rev_dist, old_VAF) -
                        logpdf(prop_dist, new_VAF)
        ...
        for j in preorder_nodes
            ...
            slack = min_VAF_in - sum(max_VAFs_out)
            if slack > 0
                # Identical to the original allocation when there is slack to share.
                unallocated_VAF = sort(rand(Uniform(0, slack), length(max_VAFs_out)+2))
                ...
                # No room to expand (slack zero, or marginally negative through
                # floating-point rounding). Pin the windows to the current mutation
                # extremes. No random numbers are drawn here, so a normal run, where
                # slack is always positive, never reaches this branch.
                br_ends[getinbound(tree,j)] = min_VAF_in
                for (k,l) in enumerate(getoutbounds(tree,j))
                    br_starts[l] = max_VAFs_out[k]
                end
            end
        end
